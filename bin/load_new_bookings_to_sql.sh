#!/bin/bash
#may have to manually load first booking for new calendar year
...
export bookings_count=$(ls ~/data/LVMPD/BOOKINGS/*$yr-ITAG-ALL*.htm | wc -l)
export bookings_loaded_count=$(ls ~/data/LVMPD/BOOKINGS/*$yr-ITAG-ALL*.htm | grep -n $last_booking | cut -d ':' -f 1)
